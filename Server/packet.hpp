...
class server_code_assign_packet : public server_packet<PT_CODE_ASSIGN> {
public:
  uint32_t code;
  std::string ip;

  server_code_assign_packet(uint32_t code, std::string ip)
      : code(code)
      , ip(std::move(ip)) {
  }

  server_code_assign_packet(buffer_reader& reader) {
    reader.read(code);
    reader.read(ip);
  }

  size_t size() const override {
    return 1 + sizeof(code) + 1 + ip.size();
  }
...
public:
  ...
  client_create_game_packet(uint32_t cookie, std::string password, uint32_t difficulty)
    : cookie(cookie)
  ...
  size_t size() const override {
    return 2 + sizeof(cookie) + password.size() + sizeof(difficulty);
  }
...
public:
  ...
  client_join_game_packet(uint32_t cookie, uint32_t code, std::string password)
      : cookie(cookie)
      , code(code)
      , password(std::move(password)) {
  }
  ...
  size_t size() const override {
    return 2 + sizeof(cookie) + sizeof(code) + password.size();
  }
