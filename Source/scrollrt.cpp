...
const char *szPlrModeAssert[12] = {
    ...
    "casting a spell", "changing levels",   "quitting"};
...
void DrawMissile(int x, int y, int sx, int sy, int CelSkip, int CelCap,
                 BOOL pre) {
  int i, mx, my, nCel;
  MissileStruct *m;
  BYTE *pCelBuff;
  ...
  if (dMissile[x][y] == -1) {
    for (i = 0; i < nummissiles; i++) {
      ...
      if (m->_mix == x && m->_miy == y && m->_miPreFlag == pre &&
          m->_miDrawFlag) {
        ...
        if (nCel < 1 || pFrameTable[0] > 50 || nCel > (int)pFrameTable[0]) {
          // app_fatal("Draw Missile: frame %d of %d, missile type==%d", nCel,
          // pFrameTable[0], m->_mitype);
          return;
        }
        mx = sx + m->_mixoff - m->_miAnimWidth2;
        my = sy + m->_miyoff;
        if (m->_miUniqTrans)
          Cl2DecodeFrm3(mx, my, m->_miAnimData, m->_miAnimFrame,
                        m->_miAnimWidth, CelSkip, CelCap, m->_miUniqTrans + 3);
        else if (m->_miLightFlag)
          Cl2DecodeLightTbl(mx, my, m->_miAnimData, m->_miAnimFrame,
                            m->_miAnimWidth, CelSkip, CelCap);
        else
          Cl2DecodeFrm1(mx, my, m->_miAnimData, m->_miAnimFrame,
                        m->_miAnimWidth, CelSkip, CelCap);
      }
    }
  } else {
    ...
    if (m->_miPreFlag == pre && m->_miDrawFlag) {
      ...
      pFrameTable = (DWORD *)pCelBuff;
      if (nCel < 1 || pFrameTable[0] > 50 || nCel > (int)pFrameTable[0]) {
        // app_fatal("Draw Missile 2: frame %d of %d, missile type==%d", nCel,
        // pFrameTable[0], m->_mitype);
        return;
      }
      mx = sx + m->_mixoff - m->_miAnimWidth2;
      my = sy + m->_miyoff;
      if (m->_miUniqTrans)
        Cl2DecodeFrm3(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth,
                      CelSkip, CelCap, m->_miUniqTrans + 3);
      else if (m->_miLightFlag)
        Cl2DecodeLightTbl(mx, my, m->_miAnimData, m->_miAnimFrame,
                          m->_miAnimWidth, CelSkip, CelCap);
      else
...
void DrawClippedMissile(int x, int y, int sx, int sy, int CelSkip, int CelCap,
                        BOOL pre) {
  int i, mx, my, nCel;
  MissileStruct *m;
  BYTE *pCelBuff;
  ...
  if (dMissile[x][y] == -1) {
    for (i = 0; i < nummissiles; i++) {
      ...
      if (m->_mix == x && m->_miy == y && m->_miPreFlag == pre &&
        ...
        pCelBuff = m->_miAnimData;
        if (!pCelBuff) {
          // app_fatal("Draw Missile type %d Clipped: NULL Cel Buffer",
          // m->_mitype);
          return;
        }
        nCel = m->_miAnimFrame;
        pFrameTable = (DWORD *)pCelBuff;
        if (nCel < 1 || pFrameTable[0] > 50 || nCel > (int)pFrameTable[0]) {
          // app_fatal("Draw Clipped Missile: frame %d of %d, missile type==%d",
          // nCel, pFrameTable[0], m->_mitype);
          return;
        }
        mx = sx + m->_mixoff - m->_miAnimWidth2;
        my = sy + m->_miyoff;
        if (m->_miUniqTrans)
          Cl2DecodeFrm5(mx, my, m->_miAnimData, m->_miAnimFrame,
                        m->_miAnimWidth, CelSkip, CelCap, m->_miUniqTrans + 3);
        else if (m->_miLightFlag)
          Cl2DecodeFrm6(mx, my, m->_miAnimData, m->_miAnimFrame,
                        m->_miAnimWidth, CelSkip, CelCap);
        else
          Cl2DecodeFrm4(mx, my, m->_miAnimData, m->_miAnimFrame,
                        m->_miAnimWidth, CelSkip, CelCap);
      }
    }
  } else {
    ...
    if (m->_miPreFlag == pre && m->_miDrawFlag) {
      pCelBuff = m->_miAnimData;
      if (!pCelBuff) {
        // app_fatal("Draw Missile 2 type %d Clipped: NULL Cel Buffer",
        // m->_mitype);
        return;
      }
      nCel = m->_miAnimFrame;
      pFrameTable = (DWORD *)pCelBuff;
      if (nCel < 1 || pFrameTable[0] > 50 || nCel > (int)pFrameTable[0]) {
        // app_fatal("Draw Clipped Missile 2: frame %d of %d, missile type==%d",
        // nCel, pFrameTable[0], m->_mitype);
        return;
      }
      mx = sx + m->_mixoff - m->_miAnimWidth2;
      my = sy + m->_miyoff;
      if (m->_miUniqTrans)
        Cl2DecodeFrm5(mx, my, m->_miAnimData, m->_miAnimFrame, m->_miAnimWidth,
                      CelSkip, CelCap, m->_miUniqTrans + 3);
      else if (m->_miLightFlag)
...
void DrawDeadPlayer(int x, int y, int sx, int sy, int CelSkip, int CelCap,
                    BOOL clipped) {
  int i, px, py, nCel;
  PlayerStruct *p;
  BYTE *pCelBuff;
  ...
  for (i = 0; i < MAX_PLRS; i++) {
    p = &plr[i];
    if (p->plractive && !p->_pHitPoints && p->plrlevel == (BYTE)currlevel &&
        p->WorldX == x && p->WorldY == y) {
      pCelBuff = p->_pAnimData;
      if (!pCelBuff) {
        // app_fatal("Drawing dead player %d \"%s\": NULL Cel Buffer", i,
        // p->_pName);
        break;
      }
      nCel = p->_pAnimFrame;
      pFrameTable = (DWORD *)pCelBuff;
      if (nCel < 1 || pFrameTable[0] > 50 || nCel > (int)pFrameTable[0]) {
        // app_fatal("Drawing dead player %d \"%s\": facing %d, frame %d of %d",
        // i, p->_pName, p->_pdir, nCel, pFrameTable[0]);
        break;
      }
      dFlags[x][y] |= BFLAG_DEAD_PLAYER;
      px = sx + p->_pxoff - p->_pAnimWidth2;
      py = sy + p->_pyoff;
      DrawPlrProc(i, x, y, px, py, p->_pAnimData, p->_pAnimFrame,
                  p->_pAnimWidth, CelSkip, CelCap);
    }
  }
}

void DrawPlayer(int pnum, int x, int y, int px, int py, BYTE *pCelBuff,
                int nCel, int nWidth, int CelSkip, int CelCap) {
  int l;
  DWORD *pFrameTable;
  ...
  if (dFlags[x][y] & BFLAG_LIT || plr[myplr]._pInfraFlag ||
      !setlevel && !currlevel) {
    if (!pCelBuff) {
      // app_fatal("Drawing player %d \"%s\": NULL Cel Buffer", pnum,
      // plr[pnum]._pName);
      return;
    }
    pFrameTable = (DWORD *)pCelBuff;
    ...
    if (pnum == myplr) {
      ...
      if (plr[pnum].pManaShield)
        Cl2DecodeFrm1(px + plr[pnum]._pAnimWidth2 -
                          misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
                      py, misfiledata[MFILE_MANASHLD].mAnimData[0], 1,
                      misfiledata[MFILE_MANASHLD].mAnimWidth[0], CelSkip,
                      CelCap);
    } else if (!(dFlags[x][y] & BFLAG_LIT) ||
               plr[myplr]._pInfraFlag && light_table_index > 8) {
      Cl2DecodeFrm3(px, py, pCelBuff, nCel, nWidth, CelSkip, CelCap, 1);
      if (plr[pnum].pManaShield)
        Cl2DecodeFrm3(px + plr[pnum]._pAnimWidth2 -
                          misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
                      py, misfiledata[MFILE_MANASHLD].mAnimData[0], 1,
                      misfiledata[MFILE_MANASHLD].mAnimWidth[0], CelSkip,
                      CelCap, 1);
      ...
      if (plr[pnum].pManaShield)
        Cl2DecodeLightTbl(px + plr[pnum]._pAnimWidth2 -
                              misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
                          py, misfiledata[MFILE_MANASHLD].mAnimData[0], 1,
                          misfiledata[MFILE_MANASHLD].mAnimWidth[0], CelSkip,
                          CelCap);
...
void DrawClippedPlayer(int pnum, int x, int y, int px, int py, BYTE *pCelBuff,
                       int nCel, int nWidth, int CelSkip, int CelCap) {
  int l;
  DWORD *pFrameTable;
  ...
  if (dFlags[x][y] & BFLAG_LIT || plr[myplr]._pInfraFlag) {
    if (!pCelBuff) {
      // app_fatal("Drawing player %d \"%s\" clipped: NULL Cel Buffer", pnum,
      // plr[pnum]._pName);
      return;
    }
    pFrameTable = (DWORD *)pCelBuff;
    ...
    if (pnum == myplr) {
      ...
      if (plr[pnum].pManaShield)
        Cl2DecodeFrm4(px + plr[pnum]._pAnimWidth2 -
                          misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
                      py, misfiledata[MFILE_MANASHLD].mAnimData[0], 1,
                      misfiledata[MFILE_MANASHLD].mAnimWidth[0], CelSkip,
                      CelCap);
      ...
      if (plr[pnum].pManaShield)
        Cl2DecodeFrm5(px + plr[pnum]._pAnimWidth2 -
                          misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
                      py, misfiledata[MFILE_MANASHLD].mAnimData[0], 1,
                      misfiledata[MFILE_MANASHLD].mAnimWidth[0], CelSkip,
                      CelCap, 1);
      ...
      if (plr[pnum].pManaShield)
        Cl2DecodeFrm6(px + plr[pnum]._pAnimWidth2 -
                          misfiledata[MFILE_MANASHLD].mAnimWidth2[0],
                      py, misfiledata[MFILE_MANASHLD].mAnimData[0], 1,
                      misfiledata[MFILE_MANASHLD].mAnimWidth[0], CelSkip,
                      CelCap);
...
void scrollrt_draw_lower(int x, int y, int sx, int sy, int chunks, int eflag) {
    ...
    if ((DWORD)y < MAXDUNY && (DWORD)x < MAXDUNX) {
      ...
      if (level_piece_id != 0) {
        ...
        cel_transparency_active =
            (BYTE)(nTransTable[level_piece_id] & TransList[dTransVal[x][y]]);
        arch_draw_type = 2;
        ...
        scrollrt_draw_clipped_dungeon(&gpBuffer[sx + PitchTbl[sy]], x, y, sx,
                                      sy, 0);
      } else {
  ...
  if (eflag && (DWORD)y < MAXDUNY && (DWORD)x < MAXDUNX) {
    ...
    if (level_piece_id == 0) {
      ...
      scrollrt_draw_clipped_dungeon(&gpBuffer[sx + PitchTbl[sy]], x, y, sx, sy,
                                    0);
    }
  }
}

void scrollrt_draw_clipped_dungeon(BYTE *pBuff, int sx, int sy, int dx, int dy,
                                   int eflag) {
      ...
      if (pCelBuff != NULL) {
        ...
        if (nCel >= 1 && pFrameTable[0] <= 50 && nCel <= (int)pFrameTable[0]) {
          if (pDeadGuy->_deadtrans != 0) {
            Cl2DecodeFrm5(px, dy, pCelBuff, nCel, pDeadGuy->_deadWidth, 0, 8,
                          pDeadGuy->_deadtrans);
          } else {
            Cl2DecodeFrm6(px, dy, pCelBuff, pDeadGuy->_deadFrame,
                          pDeadGuy->_deadWidth, 0, 8);
          }
        } else {
          // app_fatal("Clipped dead sub: frame %d of %d, deadnum==%d", nCel,
          // pFrameTable[0], (bDead & 0x1F) - 1);
        }
      }
    }
    if (bObj != 0) {
      ...
      if ((BYTE)bItem <= MAXITEMS) {
        ...
        if (pCelBuff != NULL) {
          ...
          if (nCel >= 1 && pFrameTable[0] <= 50 &&
            ...
            Cel2DecodeHdrLight(px, dy, pItem->_iAnimData, pItem->_iAnimFrame,
                               pItem->_iAnimWidth, 0, 8);
          } else {
            /*
            app_fatal(
          ...
          // app_fatal("Draw Item \"%s\" Clipped 1: NULL Cel Buffer",
          // pItem->_iIName);
        }
      }
    }
  ...
  if (bFlag & BFLAG_PLAYERLR) {
    ...
    if ((DWORD)p < MAX_PLRS) {
      ...
      DrawClippedPlayer(p, sx, sy - 1, px, py, pPlayer->_pAnimData,
                        pPlayer->_pAnimFrame, pPlayer->_pAnimWidth, 0, 8);
      if (eflag && pPlayer->_peflag != 0) {
        if (pPlayer->_peflag == 2) {
          scrollrt_draw_clipped_e_flag(pBuff - (BUFFER_WIDTH * 16 + 96), sx - 2,
                                       sy + 1, dx - 96, dy - 16);
        }
  ...
  if (bFlag & BFLAG_MONSTLR && (bFlag & BFLAG_LIT || plr[myplr]._pInfraFlag) &&
      negMon < 0) {
      ...
      if (!(pMonster->_mFlags & 1)) {
        if (pMonster->MType != NULL) {
          ...
          if (draw_monster_num == pcursmonst) {
            Cl2DecodeClrHL(233, px, py, pMonster->_mAnimData,
                           pMonster->_mAnimFrame, pMonster->MType->width, 0, 8);
          }
          DrawClippedMonster(sx, sy, px, py, draw_monster_num, 0, 8);
          if (eflag && pMonster->_meflag) {
            scrollrt_draw_clipped_e_flag(pBuff - 64, sx - 1, sy + 1, dx - 64,
                                         dy);
          }
        } else {
          // app_fatal("Draw Monster \"%s\" Clipped: uninitialized monster",
          // pMonster->mName);
        }
      }
    } else {
      // app_fatal("Draw Monster Clipped: tried to draw illegal monster %d",
      // draw_monster_num);
    }
  }
  ...
  if (bPlr > 0) {
    ...
    if ((DWORD)p < MAX_PLRS) {
      ...
      px = dx + pPlayer->_pxoff - pPlayer->_pAnimWidth2;
      py = dy + pPlayer->_pyoff;
      DrawClippedPlayer(p, sx, sy, px, py, pPlayer->_pAnimData,
                        pPlayer->_pAnimFrame, pPlayer->_pAnimWidth, 0, 8);
      if (eflag && pPlayer->_peflag != 0) {
        if (pPlayer->_peflag == 2) {
          scrollrt_draw_clipped_e_flag(pBuff - (BUFFER_WIDTH * 16 + 96), sx - 2,
    ...
    if (pItem->_iPostDraw) {
      ...
      if ((BYTE)bItem <= MAXITEMS) {
        ...
        if (pCelBuff != NULL) {
          ...
          // app_fatal("Draw Item \"%s\" Clipped 2: NULL Cel Buffer",
          // pItem->_iIName);
        }
      }
    }
...
void DrawClippedMonster(int x, int y, int mx, int my, int m, int CelSkip,
                        int CelCap) {
  int nCel;
  char trans;
  BYTE *pCelBuff;
  ...
  if (!pCelBuff) {
    // app_fatal("Draw Monster \"%s\" Clipped: NULL Cel Buffer",
    // monster[m].mName);
    return;
  }
  ...
  if (!(dFlags[x][y] & BFLAG_LIT)) {
    Cl2DecodeFrm5(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame,
                  monster[m].MType->width, CelSkip, CelCap, 1);
  } else {
    trans = 0;
    ...
    if (trans)
      Cl2DecodeFrm5(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame,
                    monster[m].MType->width, CelSkip, CelCap, trans);
    else
...
void DrawClippedObject(int x, int y, int ox, int oy, BOOL pre, int CelSkip,
                       int CelCap) {
  ...
  if (!pCelBuff) {
    // app_fatal("Draw Object type %d Clipped: NULL Cel Buffer",
    // object[bv]._otype);
    return;
  }
  ...
  pFrameTable = (DWORD *)pCelBuff;
  if (nCel < 1 || pFrameTable[0] > 50 || nCel > (int)pFrameTable[0]) {
    // app_fatal("Draw Clipped Object: frame %d of %d, object type==%d", nCel,
    // pFrameTable[0], object[bv]._otype);
    return;
  }

  if (bv == pcursobj)
    CelDrawHdrClrHL(194, sx, sy, object[bv]._oAnimData, object[bv]._oAnimFrame,
                    object[bv]._oAnimWidth, CelSkip, CelCap);
  if (object[bv]._oLight)
    Cel2DecodeHdrLight(sx, sy, object[bv]._oAnimData, object[bv]._oAnimFrame,
                       object[bv]._oAnimWidth, CelSkip, CelCap);
  else
    Cel2DrawHdrOnly(sx, sy, object[bv]._oAnimData, object[bv]._oAnimFrame,
                    object[bv]._oAnimWidth, CelSkip, CelCap);
}
...
void scrollrt_draw_lower_2(int x, int y, int sx, int sy, int chunks,
                           int skipChunks, int eflag) {
  int i, j, CelSkip;
  BYTE *dst;
  MICROS *pMap;
      ...
      if (level_piece_id != 0) {
        ...
        if (CelSkip < 8) {
          scrollrt_draw_clipped_dungeon_2(
              &gpBuffer[sx + PitchTbl[sy] - BUFFER_WIDTH * 16 * CelSkip], x, y,
              skipChunks, CelSkip, sx, sy, 0);
        }
      }
    }
    x++;
        ...
        if (CelSkip < 8) {
          scrollrt_draw_clipped_dungeon_2(
              &gpBuffer[sx + PitchTbl[sy] -
                        BUFFER_WIDTH * 32 * (skipChunks + 1)],
              x, y, skipChunks, CelSkip, sx, sy, 1);
        }
      }
    }
    x++;
...
void scrollrt_draw_clipped_dungeon_2(BYTE *pBuff, int sx, int sy,
                                     int skipChunks, int CelSkip, int dx,
                                     int dy, int eflag) {
  int px, py, nCel, nMon, negMon, p;
  char bFlag, bDead, bObj, bItem, bPlr, bArch, bMap, negPlr, dd;
  DeadStruct *pDeadGuy;
  ...
  if (light_table_index < lightmax) {
    if (bDead != 0) {
      ...
      if (pCelBuff != NULL) {
        ...
        if (nCel >= 1 && pFrameTable[0] <= 50 && nCel <= (int)pFrameTable[0]) {
          if (pDeadGuy->_deadtrans != 0) {
            Cl2DecodeFrm5(px, dy, pCelBuff, nCel, pDeadGuy->_deadWidth, CelSkip,
                          8, pDeadGuy->_deadtrans);
          } else {
            Cl2DecodeFrm6(px, dy, pCelBuff, pDeadGuy->_deadFrame,
                          pDeadGuy->_deadWidth, CelSkip, 8);
          }
        } else {
          // app_fatal("Clipped dead sub2: frame %d of %d, deadnum==%d", nCel,
          // pFrameTable[0], (bDead & 0x1F) - 1);
        }
      }
    }
    ...
    if (!pItem->_iPostDraw) {
      ...
      if ((BYTE)bItem <= MAXITEMS) {
        ...
        if (pCelBuff != NULL) {
          ...
          // app_fatal("Draw Item \"%s\" Clipped 3: NULL Cel Buffer",
          // pItem->_iIName);
        }
      }
    }
  ...
  if (bFlag & BFLAG_PLAYERLR) {
    ...
    if ((DWORD)p < MAX_PLRS) {
      ...
      py = dy + pPlayer->_pyoff;
      DrawClippedPlayer(p, sx, sy - 1, px, py, pPlayer->_pAnimData,
                        pPlayer->_pAnimFrame, pPlayer->_pAnimWidth, CelSkip, 8);
      if (eflag && pPlayer->_peflag != 0) {
        if (pPlayer->_peflag == 2) {
          scrollrt_draw_clipped_e_flag_2(pBuff - (BUFFER_WIDTH * 16 + 96),
                                         sx - 2, sy + 1, skipChunks, CelSkip,
                                         dx - 96, dy - 16);
        }
        scrollrt_draw_clipped_e_flag_2(pBuff - 64, sx - 1, sy + 1, skipChunks,
                                       CelSkip, dx - 64, dy);
      }
    } else {
      // app_fatal("draw player clipped: tried to draw illegal player %d", p);
    ...
    if ((DWORD)draw_monster_num < MAXMONSTERS) {
      ...
      if (!(pMonster->_mFlags & 1)) {
        if (pMonster->MType != NULL) {
          ...
          if (draw_monster_num == pcursmonst) {
            Cl2DecodeClrHL(233, px, py, pMonster->_mAnimData,
                           pMonster->_mAnimFrame, pMonster->MType->width,
                           CelSkip, 8);
          }
          DrawClippedMonster(sx, sy, px, py, draw_monster_num, CelSkip, 8);
          if (eflag && !pMonster->_meflag) {
            scrollrt_draw_clipped_e_flag_2(pBuff - 64, sx - 1, sy + 1,
                                           skipChunks, CelSkip, dx - 64, dy);
          }
        } else {
          // app_fatal("Draw Monster \"%s\" Clipped: uninitialized monster",
          // pMonster->mName);
    ...
    if (pItem->_iPostDraw) {
      ...
      if ((BYTE)bItem <= MAXITEMS) {
        ...
        if (pCelBuff != NULL) {
          ...
          // app_fatal("Draw Item \"%s\" Clipped 4: NULL Cel Buffer",
          // pItem->_iIName);
        }
      }
    }
...
void scrollrt_draw_clipped_e_flag_2(BYTE *pBuff, int x, int y, int skipChunks,
                                    int CelSkip, int sx, int sy) {
  ...
  if (CelSkip < 8) {
    scrollrt_draw_clipped_dungeon_2(pBuff, x, y, skipChunks, CelSkip, sx, sy,
                                    0);
  }
...
void scrollrt_draw_upper(int x, int y, int sx, int sy, int chunks,
                         int capChunks, int eflag) {
  int i, j, CelCap;
  BYTE *dst;
  MICROS *pMap;
  ...
  if (eflag) {
    if (y >= 0 && y < MAXDUNY && x >= 0 && x < MAXDUNX) {
      ...
      if (level_piece_id != 0) {
        ...
        scrollrt_draw_dungeon(&gpBuffer[sx + PitchTbl[sy]], x, y, capChunks,
                              CelCap, sx, sy, 0);
      } else {
        world_draw_black_tile(&gpBuffer[sx + PitchTbl[sy]]);
      }
...
void scrollrt_draw_dungeon(BYTE *pBuff, int sx, int sy, int capChunks,
                           int CelCap, int dx, int dy, int eflag) {
  ...
  if (light_table_index < lightmax) {
    if (bDead != 0) {
      ...
      if (pCelBuff != NULL) {
        ...
        if (nCel >= 1 && pFrameTable[0] <= 50 && nCel <= (int)pFrameTable[0]) {
          if (pDeadGuy->_deadtrans != 0) {
            Cl2DecodeFrm3(px, dy, pCelBuff, nCel, pDeadGuy->_deadWidth, 0,
                          CelCap, pDeadGuy->_deadtrans);
          } else {
            Cl2DecodeLightTbl(px, dy, pCelBuff, pDeadGuy->_deadFrame,
                              pDeadGuy->_deadWidth, 0, CelCap);
          }
        } else {
          // app_fatal("Unclipped dead: frame %d of %d, deadnum==%d", nCel,
          // pFrameTable[0], (bDead & 0x1F) - 1);
        }
      }
    }
      ...
      if ((BYTE)bItem <= MAXITEMS) {
        ...
        if (pCelBuff != NULL) {
          ...
          if (nCel >= 1 && pFrameTable[0] <= 50 &&
            ...
            px = dx - pItem->_iAnimWidth2;
            if (bItem - 1 == pcursitem) {
              CelDecodeClr(181, px, dy, pCelBuff, nCel, pItem->_iAnimWidth, 0,
                           CelCap);
            }
            CelDecodeHdrLightOnly(px, dy, pItem->_iAnimData, pItem->_iAnimFrame,
                                  pItem->_iAnimWidth, 0, CelCap);
          } else {
            // app_fatal("Draw \"%s\" Item 1: frame %d of %d, item type==%d",
            // pItem->_iIName, nCel, pFrameTable[0], pItem->_itype);
          }
        } else {
  ...
  if (bFlag & BFLAG_PLAYERLR) {
    ...
    if ((DWORD)p < MAX_PLRS) {
      ...
      px = dx + pPlayer->_pxoff - pPlayer->_pAnimWidth2;
      py = dy + pPlayer->_pyoff;
      DrawPlayer(p, sx, sy - 1, px, py, pPlayer->_pAnimData,
                 pPlayer->_pAnimFrame, pPlayer->_pAnimWidth, 0, CelCap);
      if (eflag && pPlayer->_peflag != 0) {
        if (pPlayer->_peflag == 2) {
          scrollrt_draw_e_flag(pBuff - (BUFFER_WIDTH * 16 + 96), sx - 2, sy + 1,
                               capChunks, CelCap, tx, ty);
        }
        scrollrt_draw_e_flag(pBuff - 64, sx - 1, sy + 1, capChunks, CelCap,
                             dx - 64, dy);
      }
    } else {
    ...
    if ((DWORD)draw_monster_num < MAXMONSTERS) {
      ...
      if (!(pMonster->_mFlags & 1)) {
        if (pMonster->MType != NULL) {
          ...
          py = dy + pMonster->_myoff;
          if (draw_monster_num == pcursmonst) {
            Cl2DecodeFrm2(233, px, py, pMonster->_mAnimData,
                          pMonster->_mAnimFrame, pMonster->MType->width, 0,
                          CelCap);
          }
          ...
          // app_fatal("Draw Monster \"%s\": uninitialized monster",
          // pMonster->mName);
        }
      }
    } else {
      // app_fatal("Draw Monster: tried to draw illegal monster %d",
      // draw_monster_num);
    }
  }
  if (bFlag & BFLAG_DEAD_PLAYER) {
  ...
  if (bPlr > 0) {
    ...
    if ((DWORD)p < MAX_PLRS) {
      ...
      px = dx + pPlayer->_pxoff - pPlayer->_pAnimWidth2;
      py = dy + pPlayer->_pyoff;
      DrawPlayer(p, sx, sy, px, py, pPlayer->_pAnimData, pPlayer->_pAnimFrame,
                 pPlayer->_pAnimWidth, 0, CelCap);
      if (eflag && pPlayer->_peflag != 0) {
        if (pPlayer->_peflag == 2) {
          scrollrt_draw_e_flag(pBuff - (BUFFER_WIDTH * 16 + 96), sx - 2, sy + 1,
                               capChunks, CelCap, dx - 96, dy - 16);
        }
        scrollrt_draw_e_flag(pBuff - 64, sx - 1, sy + 1, capChunks, CelCap,
                             dx - 64, dy);
      }
      ...
      if ((BYTE)bItem <= MAXITEMS) {
        ...
        if (pCelBuff != NULL) {
          ...
          if (nCel >= 1 && pFrameTable[0] <= 50 &&
            ...
            // app_fatal("Draw \"%s\" Item 2: frame %d of %d, item type==%d",
            // pItem->_iIName, nCel, pFrameTable[0], pItem->_itype);
          }
        } else {
...
void DrawMonster(int x, int y, int mx, int my, int m, int CelSkip, int CelCap) {
  ...
  if (!(dFlags[x][y] & BFLAG_LIT)) {
    Cl2DecodeFrm3(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame,
                  monster[m].MType->width, CelSkip, CelCap, 1);
  } else {
    trans = 0;
    if (monster[m]._uniqtype)
    ...
    if (trans)
      ...
      Cl2DecodeLightTbl(mx, my, monster[m]._mAnimData, monster[m]._mAnimFrame,
                        monster[m].MType->width, CelSkip, CelCap);
  }
}

void DrawObject(int x, int y, int ox, int oy, BOOL pre, int CelSkip,
                int CelCap) {
  int sx, sy, xx, yy, nCel;
  char bv;
  BYTE *pCelBuff;
  ...
  pFrameTable = (DWORD *)pCelBuff;
  if (nCel < 1 || pFrameTable[0] > 50 || nCel > (int)pFrameTable[0]) {
    // app_fatal("Draw Object: frame %d of %d, object type==%d", nCel,
    // pFrameTable[0], object[bv]._otype);
    return;
  }

  if (bv == pcursobj)
    CelDecodeClr(194, sx, sy, object[bv]._oAnimData, object[bv]._oAnimFrame,
                 object[bv]._oAnimWidth, CelSkip, CelCap);
  if (object[bv]._oLight) {
    CelDecodeHdrLightOnly(sx, sy, object[bv]._oAnimData, object[bv]._oAnimFrame,
                          object[bv]._oAnimWidth, CelSkip, CelCap);
  } else {
    /// ASSERT: assert(object[bv]._oAnimData);
    if (object[bv]._oAnimData) // BUGFIX: _oAnimData was already checked, this
                               // is redundant
      CelDrawHdrOnly(sx, sy, object[bv]._oAnimData, object[bv]._oAnimFrame,
                     object[bv]._oAnimWidth, CelSkip, CelCap);
  }
}

void scrollrt_draw_e_flag(BYTE *pBuff, int x, int y, int capChunks, int CelCap,
                          int sx, int sy) {
  int i, lti_old, cta_old, lpi_old;
  BYTE *dst;
  MICROS *pMap;
...
void DrawZoom(int x, int y) {
  ...
  for (hgt = 176; hgt != 0; hgt--, src -= BUFFER_WIDTH + wdt,
      dst1 -= 2 * (BUFFER_WIDTH + wdt), dst2 -= 2 * (BUFFER_WIDTH + wdt)) {
    for (i = wdt; i != 0; i--) {
      *dst1++ = *src;
      *dst1++ = *src;
      *dst2++ = *src;
...
void scrollrt_draw_cursor_item() {
  ...
  if (pcurs >= CURSOR_FIRSTITEM) {
    ...
    CelDrawHdrClrHL(col, mx + SCREEN_X, my + cursH + SCREEN_Y - 1, pCursCels,
                    pcurs, cursW, 0, 8);
    if (col != PAL16_RED + 5) {
      Cel2DrawHdrOnly(mx + SCREEN_X, my + cursH + SCREEN_Y - 1, pCursCels,
                      pcurs, cursW, 0, 8);
    } else {
      Cel2DrawHdrLightRed(mx + SCREEN_X, my + cursH + SCREEN_Y - 1, pCursCels,
                          pcurs, cursW, 0, 8, 1);
    }
  } else {
    Cel2DrawHdrOnly(mx + SCREEN_X, my + cursH + SCREEN_Y - 1, pCursCels, pcurs,
                    cursW, 0, 8);
  }
}

void DrawMain(int dwHgt, BOOL draw_desc, BOOL draw_hp, BOOL draw_mana,
              BOOL draw_sbar, BOOL draw_btn) {
  int ysize = dwHgt;
  if (!draw_lock(&ysize)) {
    return;
  }
